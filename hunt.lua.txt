...
function safeTweenTo(targetCFrame)
    ...
    local tween = TweenService:Create(
        part,
        TweenInfo.new(distance / 360, Enum.EasingStyle.Linear),
        {CFrame = targetCFrame + Vector3.new(0,0,0)}
    )
    tween:Play()
...
local function getFurthestZombieMesh()
    local zombies = workspace.Entities:FindFirstChild("Zombie")
    if not zombies then return nil end
    local char = plr.Character
    ...
    while true do
        ...
        if char and char:FindFirstChild("Humanoid") and char:FindFirstChild("HumanoidRootPart") then
            ...
            if hum.Health / hum.MaxHealth < 0.2 then
                targetCFrame = getFurthestZombieMesh() -- jauhi zombie
            else
                targetCFrame = getRandomZombieMesh() -- dekati zombie
            end
